...
router.get('/variant/:variantId', async (req, res) => {
    ...
    try {
        ...
        for (const q of questions.rows) {
            ...
            q.options = options.rows.map(opt => ({
                id: opt.id,
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        await client.query(
            'UPDATE questions SET question_text = $1, points = $2, variant_id = $3, image_url = $4 WHERE id = $5',
            [question_text, points || 1, variant_id, image_url || null, id]
        );
